...
for (const viewport of [{ name: "desktop", width: 1440, height: 1000 }, { name: "mobile", width: 390, height: 844 }]) {
  test(`recommendation stages and profile actions (${viewport.name})`, async ({ page }, info) => {
    ...
    const button = page.getByRole("button", { name: "추천 장소 보기", exact: true });
    ...
    const busyButton = page.locator("button.button--primary[aria-busy=true]");
    await expect(busyButton).toBeVisible();
    await expect(busyButton).toBeDisabled();
    const during = await busyButton.boundingBox();
    ...
    await expect(page.getByRole("status").filter({ hasText: "응답을 기다리고 있어요" })).toHaveCount(1);
    expect(runRequests).toBe(0);
    await expect(progress).toHaveAttribute("aria-valuenow", "0");
    releaseProfile();
    await expect(progress).toHaveAttribute("aria-valuenow", "1");
    await expect.poll(() => runRequests).toBe(1);
    await expect(busyButton.locator('[data-state="done"]')).toHaveCount(1);
    await busyButton.scrollIntoViewIfNeeded();
    await page.screenshot({ path: info.outputPath("progress.png"), fullPage: true });
    await busyButton.screenshot({ path: info.outputPath("progress-panel.png") });
    ...
    await button.click(); await expect(progress).toHaveAttribute("aria-valuenow", "0");
    await expect(progress).toHaveAttribute("aria-valuenow", "0");
