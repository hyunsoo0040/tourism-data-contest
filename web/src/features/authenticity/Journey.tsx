...
export function ResultReason({detail}: {detail:Detail|undefined}) {
  ...
  for(const component of ordered){
    const facet=detail.facets.find(f=>f.key===component.facet || f.key.startsWith(component.facet + "."));
    if(!facet)continue;
...
export function DetailPage({ scenario = false }: { scenario?: boolean } = {}) {
  const params=useParams();const placeId=params.placeId;const runId=scenario?scenarioRunId(params.runId):params.runId;const resultHref=scenario?scenarioResultsPath(runId!):`/trip/results/${runId}`;const [detail,setDetail]=useState<Detail|null>(null),[error,setError]=useState<string|null>(null),[visited,setVisited]=useState(false),[note,setNote]=useState(""),[sent,setSent]=useState(false);
