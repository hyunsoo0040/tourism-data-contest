...
import { api, escapeId, type Detail, type Intent, type Run } from "./api";
...
function ScenarioPlace({ item, runId, compared, compareFull, onCompare, preparedDetail, failedPhotoUrls, preparedImages }: {
  item: Run["items"][number]; runId: string; compared: boolean; compareFull: boolean;
  onCompare: () => void;
...
function ScenarioResults({ runId }: { runId: string | null }) {
  ...
  useEffect(() => {
    ...
    const controller = new AbortController(); setRun(null); setProfile(null); setError(null); setCompare([]);
  ...
  return <RecommendationShell><article className="recommendations-page panel" data-status="success">
    ...
    {error && <section role="alert"><p>{error}</p><button className="control" onClick={() => { setError(null); setAttempt(value => value + 1); }}>다시 불러오기</button></section>}
    ...
    <section id="recommendation-list" className="recommendation-list" aria-label="추천 장소">{run?.items.map(item => <ScenarioPlace key={`${runId}:${item.place_id}`} item={item} runId={runId!} preparedDetail={attempt === 0 ? prepared?.details[item.place_id] : undefined} preparedImages={attempt === 0 ? prepared?.images[item.place_id] : undefined} failedPhotoUrls={prepared?.failedPhotoUrls} compared={compare.includes(item.place_id)} compareFull={compare.length >= 3} onCompare={() => setCompare(value => value.includes(item.place_id) ? value.filter(id => id !== item.place_id) : value.length < 3 ? [...value, item.place_id] : value)} />)}</section>
